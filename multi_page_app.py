...
# Page configuration
st.set_page_config(
    page_title="AI Moon - Multi-Tool Application",
    page_icon="🌙",
    layout="wide",
    ...
    with col2:
        ...
        if uploaded_image and st.button("Detect Defects", type="primary", use_container_width=True):
            with st.spinner("Analyzing image for defects..."):
                ...
                # Display results
                display_defect_results(image, results, detection_type, sensitivity)
        else:
            st.info("Upload an image and click 'Detect Defects' to analyze")

def display_defect_results(image, results, detection_type, sensitivity):
    """Display defect detection results with visualizations"""
...
def main():
    ...
    Built with Streamlit, OpenCV, and advanced computer vision techniques.
    """)
